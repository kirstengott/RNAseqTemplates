...
params:
  workingdir: ''
  args_file: 'workspace_images/DESeq2All.Rdata'
---
...
load(params$args_file)
setwd(params$workingdir)


## First list the R libraries I want to use
libraries        <- c('ggplot2', "DESeq2", "matrixStats", "gplots", "RColorBrewer", "parallel", "lattice", "plyr", "gtools", "dplyr", 'tidyr', 'kirsten', 'knitr', 'gridExtra', 'grid')
...
ggplot(data = pca_rot, aes(x = PC1, y = PC2, label = Sample, colour = factor(Condition))) +
          geom_point(size = 1) + theme_bw() + geom_text(hjust = 0, vjust = 0, size = 3) +
          xlab("PC 1") + ylab("PC 2") + ggtitle('All Samples PCA') + 
...
ggplot(data = pca_rot, aes(x = PC2, y = PC3, label = Sample, colour = factor(Condition))) +
          geom_point(size = 1) + theme_bw() + geom_text(hjust = 0, vjust = 0, size = 3) +
          xlab("PC 2") + ylab("PC 3") + ggtitle('All Samples PCA') + 
          theme(legend.position="none") + scale_x_continuous(expand = c(0.2, 0)) 
...
ggplot(importanceDF, aes(y = Value, x = Component)) + geom_bar(stat = 'identity', fill = 'purple') + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ggtitle('Principle Component Proportion of Variance') + ylab('Proportion of Variance')
